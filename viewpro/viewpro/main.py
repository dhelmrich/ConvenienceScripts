...
def import_projects_cli(source_path: str):
    ...
    if "projects" in data:
        ...
        for project in data["projects"]:
            ...
            logger.debug(
                f"import_projects_cli: processing project '{title}', logo: {logo}"
            )
    ...
    logger.info(
        f"import_projects_cli: successfully imported {imported_count} project(s)"
    )
...
def main():
    ...
    parser.add_argument(
        "--add", "-a", metavar="PATH", help="Add project by path (CLI mode)"
    )
    parser.add_argument(
        "--title", "-t", metavar="TITLE", help="Project title (use with --add)"
    )
    parser.add_argument(
        "--description",
        "-d",
        metavar="DESC",
        help="Project description (use with --add)",
    )
    parser.add_argument(
        "--logo", "-l", metavar="LOGO", help="Logo image path (use with --add)"
    )
    parser.add_argument(
        "--import",
        "-i",
