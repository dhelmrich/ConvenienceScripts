...
class ProjectCard(QFrame):
    ...
    def __init__(self, project: dict, parent_window, parent=None):
        ...
        logger.debug(f"ProjectCard created for project: {project.get('title', 'Unknown')}")
    ...
    def mouseDoubleClickEvent(self, event):
        ...
        logger.debug(f"Mouse double click on card: {self.project.get('title', 'Unknown')}, button: {event.button()}")
        if event.button() == Qt.MouseButton.LeftButton:
            logger.debug(f"Opening project: {self.project.get('title', 'Unknown')}")
            self._parent_window.open_project(self.project)
    
    def _check_close_window(self, event):
        """Check if window should close after click."""
        if not QApplication.keyboardModifiers() & Qt.KeyboardModifier.ControlModifier:
            if self._close_timer:
                self._close_timer.stop()
            self._close_timer = QTimer()
            self._close_timer.setSingleShot(True)
            self._close_timer.timeout.connect(self._parent_window.close)
            self._close_timer.start(3000)
    
    def mousePressEvent(self, event):
        logger.debug(f"Mouse press on card: {self.project.get('title', 'Unknown')}, button: {event.button()}")
        if event.button() == Qt.MouseButton.LeftButton:
            logger.debug(f"Opening project: {self.project.get('title', 'Unknown')}")
            self._parent_window.open_project(self.project)
            self._check_close_window(event)
    ...
    def _edit_project(self):
        ...
        if dialog.exec():
            ...
            if title:
                ...
                self._parent_window.project_manager.store.save({
                    'projects': projects
                })
    ...
    def _update_edit_button(self, window):
        ...
        selected_count = sum(1 for i in range(window.projects_layout.count()) 
                           if window.projects_layout.itemAt(i) and 
                           window.projects_layout.itemAt(i).widget() and 
                           window.projects_layout.itemAt(i).widget().is_selected())
...
class MainWindow(QMainWindow):
        ...
        if len(selected_cards) == 1:
            ...
            if dialog.exec():
                ...
                if title:
                    ...
                    self.project_manager.store.save({
                        'projects': projects
                    })
    ...
    def _update_edit_button(self):
        """Update edit button state based on selection."""
        selected_count = sum(1 for i in range(self.projects_layout.count()) 
                           if self.projects_layout.itemAt(i) and 
                           self.projects_layout.itemAt(i).widget() and 
                           self.projects_layout.itemAt(i).widget().is_selected())
...
class AddProjectDialog(QDialog):
    ...
    def _browse_logo(self):
        ...
        logo_path, _ = QFileDialog.getOpenFileName(self, "Select Logo Image", "", filters)
        if logo_path:
